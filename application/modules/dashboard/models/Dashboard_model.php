...
class Dashboard_model extends CI_Model {
    ...
    public function getDashboardData()
    {
        ...
        $sql    = "SELECT COUNT(kode_konfirmasi) AS permintaan_harga FROM TB_S_MST_KONFIRMASI WHERE konfirmasi_status = 2 AND tanggal_kirim = '".date('Y-m-d')."' AND kode_vendor = '{$vendor_code}' AND modified_date IS NULL AND modified_by IS NULL AND (flag_kirim IS NULL OR flag_kirim = 0)";
        $query  = $this->db->query($sql);
        $result = $query->row();
        $graph_data['permintaan_harga'] = (int)$result->permintaan_harga;

        // Total Konfirmasi Harga
        $sql    = "SELECT COUNT(kode_konfirmasi) AS konfirmasi_harga FROM TB_S_MST_KONFIRMASI WHERE konfirmasi_status = 1 AND tanggal_kirim = '".date('Y-m-d')."' AND kode_vendor = '{$vendor_code}' AND modified_date IS NULL AND modified_by IS NULL AND (flag_kirim IS NULL OR flag_kirim = 0)";
        $query  = $this->db->query($sql);
        $result = $query->row();
